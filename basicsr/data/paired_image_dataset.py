...
from basicsr.data.data_util import (paired_paths_from_folder,
                                    ...
                                    nightderaining_paired_paths_from_folder)
    ...
    def __init__(self, opt):
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            ...
            self.paths = paired_paths_from_folder(
                [self.lq_folder, self.gt_folder], ['lq', 'gt'],
                self.filename_tmpl,self.opt['phase'])
...
class Dataset_Night_Deraining(data.Dataset):
    """Paired image dataset for image restoration.
    ...
    def __init__(self, opt):
        super(Dataset_Night_Deraining, self).__init__()
        self.opt = opt
        # file client (io backend)
        self.file_client = None
        self.io_backend_opt = opt['io_backend']
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            ...
            self.paths = nightderaining_paired_paths_from_folder(
                [self.lq_folder, self.gt_folder], ['lq', 'gt'],
                self.filename_tmpl,self.opt['phase'])
