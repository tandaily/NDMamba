...
def paired_paths_from_folder(folders, keys, filename_tmpl,phase):
...
def nightderaining_paired_paths_from_folder(folders, keys, filename_tmpl,phase):
    """Generate paired paths from folders.
    ...
    for idx in range(len(input_paths)):
        input_path = input_paths[idx]
        basename_input, ext_input = osp.splitext(osp.basename(input_path))
        basename_gt = basename_input.split('_')[0]
        gt_name = f'{filename_tmpl.format(basename_gt)}{ext_input}'
