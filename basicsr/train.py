...
def init_loggers(opt):
    ...
    if opt['logger'].get('use_tb_logger') and 'debug' not in opt['name']:
        tb_logger = init_tb_logger(log_dir=osp.join('/mnt/NDMamba/logs/tb_logger', opt['name']))
...
def main():
    ...
    # automatic resume ..
    state_folder_path = '/mnt/NDMamba/logs/experiments/{}/training_states/'.format(opt['name'])
    import os
    try:
    ...
    if resume_state is None:
        ...
        if opt['logger'].get('use_tb_logger') and 'debug' not in opt[
                'name'] and opt['rank'] == 0:
            mkdir_and_rename(osp.join('/mnt/NDMamba/logs/tb_logger', opt['name']))
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            # model.optimize_parameters(current_iter,train_set)
